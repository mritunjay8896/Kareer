      ...
      <div className="max-w-[1340px] mx-auto text-center">
          ...
          {tags.map((tag) => {
            ...
            return (
              <button
                ...
                className="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl px-3 py-2 shadow-2xs hover:shadow-md transition-all cursor-pointer flex items-center gap-2.5 text-left group"
              >
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center border shrink-0 ${tag.color}`}>
                  <Icon className="w-3.5 h-3.5 stroke-[2.2]" />
                </div>
                <div className="text-left">
                  <div className="text-xs font-extrabold text-slate-900 leading-tight group-hover:text-blue-600 transition-colors">
                    {tag.label}
                  </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSearch} className="flex-1 min-w-[280px] max-w-md">
            <div className="relative flex items-center bg-white border border-slate-200/90 rounded-2xl p-1 shadow-2xs focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              ...
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs px-3.5 py-2 rounded-xl transition-colors flex items-center gap-1 shrink-0 shadow-xs cursor-pointer"
              >
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-600">
            <span className="font-bold text-slate-700 mr-0.5">Popular Searches:</span>
            {popularSearches.map((item) => (
              <button
                key={item}
                onClick={() => navigate(`/jobs?search=${encodeURIComponent(item)}`)}
                className="px-3 py-1 bg-white border border-slate-200/90 hover:border-blue-400 hover:text-blue-600 rounded-full text-xs font-medium text-slate-600 shadow-2xs transition-all cursor-pointer"
              >
                {item}
              </button>
            ))}
