...
const SAMPLE_JOB_JSON = [
  {
    ...
    "vacancyDetails": {
      "totalVacancy": 14000,
      "note": "Combined Graduate Level Examination 2026"
    },
    "importantDates": {
      "notificationDate": "10 Aug 2026",
      "applicationStart": "10 Aug 2026",
      "applicationLastDate": "15 Sep 2026",
...
export const BulkJsonUploadModal: React.FC<BulkJsonUploadModalProps> = ({
  ...
  // Helper to normalize any incoming item
  const processJobItem = (item: any, label: string): { file?: string; data: Partial<GovernmentJob>; isValid: boolean; error?: string } => {
    if (!item || typeof item !== 'object') {
      return { file: label, data: {}, isValid: false, error: 'Invalid JSON object' };
    }

    const title = item.title || item.job_title || item.page_title;
    if (!title && !item.id && !item.slug) {
      return { file: label, data: item, isValid: false, error: 'Missing title, job_title or id property' };
    }

    try {
      const normalized = normalizeGovernmentJob(item);
      return { file: label, data: normalized, isValid: true };
    } catch (e: any) {
      return { file: label, data: item, isValid: false, error: e.message || 'Normalization failed' };
    }
  };
      ...
      try {
        ...
        if (Array.isArray(json)) {
          json.forEach((item, idx) => {
            newParsed.push(processJobItem(item, `${file.name} [#${idx + 1}]`));
          });
        } else if (json && typeof json === 'object') {
          if (json.jobs && Array.isArray(json.jobs)) {
            json.jobs.forEach((item: any, idx: number) => {
              newParsed.push(processJobItem(item, `${file.name} [job #${idx + 1}]`));
            });
          } else {
            newParsed.push(processJobItem(json, file.name));
          }
        } else {
  ...
  const handleParsePastedJson = () => {
    ...
    try {
      ...
      if (Array.isArray(json)) {
        json.forEach((item, idx) => {
          items.push(processJobItem(item, `Pasted Record #${idx + 1}`));
        });
      } else if (json && typeof json === 'object') {
        if (json.jobs && Array.isArray(json.jobs)) {
          json.jobs.forEach((item: any, idx: number) => {
            items.push(processJobItem(item, `Pasted Job #${idx + 1}`));
          });
        } else {
          items.push(processJobItem(json, 'Pasted Job Notification'));
        }
      } else {
