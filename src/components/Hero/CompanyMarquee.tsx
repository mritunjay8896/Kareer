    ...
    <div className="w-full bg-slate-50/70 border-y border-slate-200/80 py-8 relative">
      <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-16 mx-auto space-y-6">
        ...
        <div className="relative group/carousel">
          ...
          {/* Cards Container - Masonry on Mobile, Flex Carousel on Desktop */}
          <div
            ref={scrollContainerRef}
            className="columns-2 gap-2.5 space-y-2.5 sm:space-y-0 sm:grid sm:grid-cols-2 lg:flex lg:space-y-0 lg:items-stretch lg:overflow-x-auto lg:scroll-smooth lg:snap-x lg:snap-mandatory pb-3 pt-1 no-scrollbar scrollbar-none px-0.5"
            style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
          >
            {campaignBanners.map((banner, idx) => (
              <div
                key={idx}
                className={`break-inside-avoid w-full lg:w-[calc((100%-2.5rem)/3)] lg:shrink-0 lg:snap-start min-h-[150px] sm:min-h-[235px] ${banner.cardBg} ${banner.borderColor} border rounded-xl sm:rounded-3xl p-3 sm:p-6 shadow-xs hover:shadow-md flex flex-col justify-between relative overflow-hidden group/card transition-all duration-300 hover:-translate-y-1`}
              >
                ...
                <div className="flex flex-col justify-between h-full relative z-10">
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-1.5 sm:mb-3">
                      <span className={`text-[8.5px] sm:text-[10px] font-bold uppercase tracking-wider px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md ${banner.badgeStyle}`}>
                        {banner.type}
                      </span>
                    </div>

                    {/* Banner Headline */}
                    <h3 className={`text-[11.5px] sm:text-xl font-black tracking-tight ${banner.titleColor} mb-1 sm:mb-2 leading-tight sm:leading-snug font-display transition-colors`}>
                      {banner.title}
                    </h3>

                    {/* Description */}
                    <p className={`text-[9.5px] sm:text-xs ${banner.descColor} leading-tight sm:leading-relaxed font-normal mb-1.5 sm:mb-3 line-clamp-2 sm:line-clamp-none`}>
                      {banner.description}
                    </p>
                  </div>
                  ...
                  <div className="pt-1 sm:pt-2">
                    <button className={`px-2.5 sm:px-5 py-1 sm:py-2.5 rounded-md sm:rounded-xl text-[9.5px] sm:text-xs font-semibold tracking-wide flex items-center gap-1 sm:gap-2 transition-all active:scale-95 ${banner.buttonStyle}`}>
                      <span>{banner.cta}</span>
                      <ArrowRight className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 transition-transform group-hover/card:translate-x-1" />
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div className="pt-1.5 sm:pt-2 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs max-w-xl sm:max-w-none mx-auto">
          <span className="font-bold text-slate-500 text-[10px] sm:text-xs tracking-wide uppercase font-display shrink-0 w-full text-center sm:w-auto mb-0.5 sm:mb-0">
            Trending Searches:
          </span>
          {trendingTags.map((tag, idx) => (
            <span
              key={idx}
              className="px-2.5 sm:px-3.5 py-1 sm:py-1.5 bg-white hover:bg-blue-50 text-slate-700 hover:text-blue-600 font-medium text-[10px] sm:text-xs rounded-full border border-slate-200/90 shadow-2xs cursor-pointer transition-all whitespace-nowrap"
            >
              {tag}
            </span>
          ))}
