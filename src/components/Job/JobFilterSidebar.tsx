...
export const JobFilterSidebar: React.FC<JobFilterSidebarProps> = ({
  ...
  // Only one filter section open at a time, rest collapsed
  const [openSection, setOpenSection] = useState<string | null>('workModes');
  ...
  const content = (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-3.5 xl:p-4 space-y-4 xl:space-y-5 shadow-xs w-full">
      {/* Header */}
      <div className="flex items-center justify-between pb-3.5 border-b border-slate-100">
        ...
        <button
          onClick={onReset}
          className="text-xs font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-1 transition-colors shrink-0"
        >
      ...
      <div className="space-y-3">
        ...
        >
          <span>Work Mode</span>
          {openSection === 'workModes' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>

        {openSection === 'workModes' && (
      ...
      <div className="space-y-3 pt-3 border-t border-slate-100">
        ...
        >
          ...
          {openSection === 'experiences' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>
      ...
      <div className="space-y-3 pt-3 border-t border-slate-100">
        ...
        >
          ...
          {openSection === 'salaryRanges' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>

        {openSection === 'salaryRanges' && (
          <div className="space-y-2">
      ...
      <div className="space-y-3 pt-3 border-t border-slate-100">
        ...
        >
          ...
          {openSection === 'departments' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>

        {openSection === 'departments' && (
          <div className="space-y-2">
            {['IT', 'Data Science', 'Marketing', 'Finance', 'Engineering', 'Government', 'Women'].map((dept) => {
      ...
      <div className="space-y-3 pt-3 border-t border-slate-100">
        ...
        >
          ...
          {openSection === 'postedWithin' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>

        {openSection === 'postedWithin' && (
          <div className="space-y-2">
            {[
  ...
  return (
    <>
      {/* Desktop Sidebar (15% width) */}
      <div className={`hidden lg:block w-full lg:w-[15%] shrink-0 sticky top-24 ${className}`}>
        {content}
      </div>
