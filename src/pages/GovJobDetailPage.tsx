        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            <div id="vacancy" className="scroll-mt-28 space-y-5">
              ...
              {/* Rich Scraped Recruitment Tables from Sarkari Result format */}
              {job.all_tables && job.all_tables.length > 0 && (
                <section className="space-y-4">
                  {job.all_tables.map((table, tIdx) => (
                    <div key={tIdx} className="bg-slate-50/80 p-4 sm:p-5 rounded-xl border border-slate-200 shadow-2xs">
                      <h2 className="text-xs sm:text-sm font-bold text-slate-900 mb-2.5 flex items-center gap-1.5 border-b border-slate-200 pb-2">
                        <FileText className="w-4 h-4 text-blue-600" />
                        <span>{table.title || `Recruitment Table #${table.table_number || tIdx + 1}`}</span>
                      </h2>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-[11px] border-collapse">
                          <tbody>
                            {table.rows.map((row, rIdx) => (
                              <tr 
                                key={rIdx} 
                                className={`border-b border-slate-200/80 ${rIdx === 0 && row.length > 2 ? 'bg-blue-100/60 font-bold text-slate-800' : 'hover:bg-blue-50/40 bg-white/70'}`}
                              >
                                {row.map((cell, cIdx) => (
                                  <td 
                                    key={cIdx} 
                                    className="py-2.5 px-3 text-slate-700 whitespace-pre-line leading-relaxed font-medium align-top"
                                  >
                                    {cell}
                                  </td>
                                ))}
